UNIT TADGrafoNDNP;
{Grafo No Dirigido y No Ponderado - Implementación Dinámica}
...
IMPLEMENTATION
	...
	PROCEDURE ConstruirListaDestino(VAR listaDestino: TipoListaDestino; destino: TipoElemento);
	VAR
	...
	BEGIN
		...
		IF (NOT PerteneceAListaDestino(pOrigen^.adyacentes, destino)) THEN
			ConstruirListaDestino(pOrigen^.adyacentes, destino);
		
		pOrigen := ReferenciaAOrigenes(g, destino);
		IF (NOT PerteneceAListaDestino(pOrigen^.adyacentes, origen)) THEN
			ConstruirListaDestino(pOrigen^.adyacentes, origen);	
	END;
	...
	BEGIN
		...
		pOrigen := ReferenciaAOrigenes(g, destino);
		IF (pOrigen <> NIL) THEN
			BorrarEnListaDestino(pOrigen^.adyacentes, origen);
	END;
